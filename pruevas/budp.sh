...
if [ "$pid_badvpn" = "" ]; then
	...
	msg -bar
    echo -ne "activar BADVPN-(UDP:7300) despues de un reinicio [s/n]: "
    read ini_udpg
    [[ $ini_udpg = @(s|S|y|Y) ]] && {
        echo 'screen -dmS badvpn2 /bin/badvpn-udpgw --listen-addr 127.0.0.1:7300 --max-clients 1000 --max-connections-for-client 10' >> /bin/cron
        chmod 777 /bin/cron
    }
    msg -bar
else
