...
echo -e "#sslh install\n\nRUN=yes\n\nDAEMON=/usr/sbin/sslh\n\nDAEMON_OPTS='--user sslh --listen 0.0.0.0:443 --anyprot 127.0.0.1:000 --ssh 127.0.0.1:22 --pidfile /var/run/sslh/sslh.pid'" > $dir_conf
#INICIAR
service sslh start
}
msg(){
    ...
    echo -e "${blanco}#   -----------------------------------------------   #${cierre}"
    echo -e "${blanco}#     ${amarillo} SCRIPT GRATUITO SOLO CON EL FIN DE AYUDAR      ${blanco}#${cierre}"
