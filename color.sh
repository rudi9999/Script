...
if [[ ! -z $ipdns ]]; then
echo -e "$BARRA"
echo -e "\033[1;31mATENCION:\n\033[1;34mPara Utilizar Este Proxy Es Necesario Agregar Una Linea A Su Payload\n\033[0m"
echo -e "\033[1;34mAGREGUE ESTA LINEA A SU PAYLOAD: \033[1;36m[crlf]X-Pass: $ipdns[crlf]\n\033[0m"
echo -e "\033[1;31mEJEMPLO 1:\n\033[1;33m\033[1;36m[crlf]X-Pass: $ipdns[crlf]GET http://tuhost.com/ HTTP/1.0 [cr|f]\033[0m"
echo -e "\033[1;31mEJEMPLO 2:\n\033[1;33m\033[1;36mGET http://tuhost.com/ HTTP/1.0 [crlf][crlf]X-Pass: $ipdns[crlf]\n\033[0m"
fi
fi
}
...
crontab_fun () {
dir=$(pwd)
cat <(crontab -l) <(echo "@reboot source $dir/color.sh; port_proxy $port; port_local $portlocal; pass_pay $ipdns; msj $mensage; color $cor;") | crontab -
}

persistente () {
	echo -n "aplicar al inicio S/N: "
	read opcion
	case $opcion in
		s) crontab_fun ;;
		n) ;;
